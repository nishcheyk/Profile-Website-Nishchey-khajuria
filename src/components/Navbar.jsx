...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#17153B] bg-opacity-50 p-4 text-white fixed w-full z-10 top-0" style={{ cursor: 'default' }}>
      <div className="mx-auto text-center flex w-5/6 justify-between items-center">
        ...
        <div className={`sm:hidden fixed top-0 left-0 w-full h-full bg-gray-800 text-center ${isMobileMenuOpen ? 'flex flex-col items-center justify-center' : 'hidden'}`}>
          ...
          <ul className="font-medium text-2xl space-y-4">
            <li><Link to='/' onClick={closeMobileMenu} className="mx-4">Home</Link></li>
            <li><Link to='/about' onClick={closeMobileMenu} className="mx-4">About</Link></li>
            <li><Link to='/projects' onClick={closeMobileMenu} className="mx-4">Projects</Link></li>
            <li><Link to='/experience' onClick={closeMobileMenu} className="mx-4">Experience</Link></li>
            <li><Link to='/contact' onClick={closeMobileMenu} className="mx-4">Contact</Link></li>
          </ul>
        </div>
      </div>
    </nav>
